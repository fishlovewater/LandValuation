...
describe('valuation demo flow', () => {
  ...
  it.skip('keeps the retired legacy F03 calculation flow for historical reference', async () => {
    ...
    expect(submitReadiness.text()).toContain('送審準備')
    expect(submitReadiness.text()).toContain('確認完整查估書')
    expect(submitReadiness.text()).toContain('完成送審文件檢核')
    expect(submitReadiness.text()).toContain('產生正式送審 PDF')
    ...
    expect(wrapper.get('[data-testid="submit-readiness-steps"]').text()).toContain('4 / 4 已完成')
