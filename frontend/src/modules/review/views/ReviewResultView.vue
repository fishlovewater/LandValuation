  ...
  <section class="review-result" data-testid="review-result">
    ...
    <template v-else-if="detail">
      <div class="review-result__toolbar">
        <div v-liquid-glass data-lg class="lg">
          <span>案件狀態</span>
      ...
      <section v-liquid-glass data-lg class="review-result__report lg" aria-labelledby="review-report-title">
        <div class="review-result__heading">
          ...
          <div class="review-result__actions">
            <GlassButton data-testid="generate-review-pdf" :disabled="!canUseReport || busy" :title="reportActionReason" @click="generatePdf">產生 PDF</GlassButton>
            <GlassButton data-testid="generate-review-xlsx" :disabled="!canUseReport || busy" :title="reportActionReason" @click="generateFormat('xlsx')">產生 Excel</GlassButton>
            <GlassButton data-testid="generate-review-docx" :disabled="!canUseReport || busy" :title="reportActionReason" @click="generateFormat('docx')">產生 Word</GlassButton>
            <GlassButton data-testid="download-review-report" variant="accent" :disabled="!reportDocument || !canUseReport || busy" :title="reportActionReason" @click="downloadReport">下載報告</GlassButton>
          </div>
        </div>
      ...
      <section v-liquid-glass data-lg class="review-result__findings lg" aria-labelledby="result-findings-title">
...
.review-result__findings { margin-top: 26px; padding: 20px; border: 1px solid rgba(255,255,255,.72); border-radius: var(--app-radius-md); background: rgba(255,255,255,.72); box-shadow: var(--app-shadow-soft); }
