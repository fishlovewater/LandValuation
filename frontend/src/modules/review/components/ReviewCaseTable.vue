...
<template>
  <div class="table-wrap">
    <table class="case-table">
      <thead><tr><th>案件</th><th>地區</th><th>狀態</th><th>風險</th><th>缺件</th><th>更新時間</th><th><span class="sr-only">操作</span></th></tr></thead>
      <tbody>
        <tr v-for="item in items" :key="item.reviewId">
          ...
          <td>{{ item.missingItemCount }}</td><td>{{ formatDateTime(item.updatedAt) }}</td>
