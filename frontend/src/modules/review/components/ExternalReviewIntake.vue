...
const pendingCandidateStatuses = new Set(['EXTRACTED', 'NEEDS_CONFIRMATION', 'CONFIRMED'])
const processedCandidateStatuses = new Set(['APPLIED', 'AUTO_APPLIED', 'REJECTED'])

const filteredCandidates = computed(() => {
  const rows = extraction.value?.candidates ?? []
  if (candidateFilter.value === 'pending') return rows.filter((item) => pendingCandidateStatuses.has(item.field_status))
  if (candidateFilter.value === 'low-confidence') return rows.filter((item) => Number(item.confidence) < 0.75)
  if (candidateFilter.value === 'confirmed') return rows.filter((item) => processedCandidateStatuses.has(item.field_status))
  return rows
})

const pendingCount = computed(() => extraction.value?.candidates.filter((item) => pendingCandidateStatuses.has(item.field_status)).length ?? 0)
const processedCount = computed(() => extraction.value?.candidates.filter((item) => processedCandidateStatuses.has(item.field_status)).length ?? 0)
const confirmedPendingApplyCount = computed(() => extraction.value?.candidates.filter((item) => item.field_status === 'CONFIRMED').length ?? 0)
...
function candidateStatusLabel(value: string): string {
  ...
  if (value === 'CONFIRMED') return '已確認，待填入表單'
...
async function startExtraction(): Promise<void> {
  ...
  try {
    ...
    notice.value = extraction.value.extraction_status === 'COMPLETED'
      ? `辨識完成：${autoCount.value} 欄自動填表，${pendingCount.value} 欄尚待確認／填表或排除。`
      : '文件擷取未完成，請確認文件內容後再試。'
    emit('changed')
  } catch (caught: unknown) {
    error.value = safeReviewErrorMessage(caught)
...
async function confirmPending(): Promise<void> {
  ...
  const pending = extraction.value?.candidates.filter(item => pendingCandidateStatuses.has(item.field_status)).slice(0, 100) ?? []
  ...
  <section class="external-intake" data-testid="external-review-intake" aria-labelledby="external-intake-title">
    <header class="external-intake__heading">
      ...
      <div class="external-intake__summary">
        ...
        <span v-if="extraction"><b>{{ pendingCount }}</b> 欄未完成</span>
        <span v-if="extraction"><b>{{ processedCount }}</b> 欄已處理</span>
      </div>
    </header>
    <p v-if="extraction" class="external-intake__scope-note">
      此處欄位數僅代表目前選取文件；開始智慧審查時，後端會檢查案件全部有效文件。
      <template v-if="confirmedPendingApplyCount">其中 {{ confirmedPendingApplyCount }} 欄已確認但尚未成功填入表單，仍需完成填表。</template>
    </p>
          ...
          <template v-if="extraction.extraction_status === 'COMPLETED'">
            <div class="external-intake__candidate-toolbar">
              ...
              <div class="external-intake__filters" aria-label="欄位候選篩選">
                ...
                <button type="button" :class="{ 'is-active': candidateFilter === 'pending' }" @click="candidateFilter = 'pending'">待完成 {{ pendingCount }}</button>
                <button type="button" :class="{ 'is-active': candidateFilter === 'low-confidence' }" @click="candidateFilter = 'low-confidence'">低信心</button>
                <button type="button" :class="{ 'is-active': candidateFilter === 'confirmed' }" @click="candidateFilter = 'confirmed'">已處理 {{ processedCount }}</button>
              </div>
            </div>
...
.external-intake__scope-note{margin:-8px 0 0;color:var(--app-muted);font-size:9px;line-height:1.55}
