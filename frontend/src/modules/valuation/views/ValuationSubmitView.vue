  ...
  try {
    ...
    if (reportProgressDto.report_id) {
      ...
      const restoredReport = formalStatus.report
        ...
        : null
      const restoredTemplateExports = (formalStatus.template_exports ?? []).map(mapTemplateExportResponse)
...
async function submitForReview(): Promise<void> {
  ...
  try {
    ...
    const result = await valuationApi.submitForReview(requestedCaseId, {
      ...
      source_validation_run_id: flow.formalValidation!.validationRunId,
      source_report_document_id: submittingExcel ? mainExcel.documentId : formalReport!.documentId,
      ...(submittingExcel
        ? { source_template_document_ids: flow.templateExports.map((item) => item.documentId) }
        : {}),
    })
    if (!isCurrentCase(token, requestedCaseId)) return
    submitRequestId.value = null
