...
const props = defineProps<{
  ...
  activeLocation: ValuationLocationDto | null
  activeLocationParcel: ParcelResponseDto | null
  ...
  >
    ...
    <div class="land-context__grid">
      <section class="land-context__panel">
        ...
        <p class="land-context__help">目前編輯：{{ props.activeLocation ? `地點 ${props.activeLocation.display_order}｜${props.activeLocation.label}` : '請先回第二步選擇地點' }}。新增宗地會直接綁定目前地點，不需再次選擇來源宗地。</p>
      ...
      <section class="land-context__panel">
        <div class="land-context__panel-heading"><strong>比準地資料</strong><span>比準地由第二步指定，不重複選擇宗地</span></div>
        <p class="land-context__help" v-if="props.activeLocation?.is_benchmark_location">目前「{{ props.activeLocation.label }}」已在第二步指定為比準地。下方資料會自動使用該地點的宗地。</p>
        <p class="land-context__help" v-else>目前地點是宗地／待估地。請在第二步選定一個地點為比準地；其餘地點不需建立比準地紀錄。</p>
        ...
        <form v-if="props.activeLocation?.is_benchmark_location && props.activeLocationParcel" class="land-context__form" @submit.prevent="emit('saveBenchmark')">
          <h3>確認比準地必要資料</h3>
          <div class="land-context__fields">
            <label><span>第二步指定的來源宗地</span><output>{{ props.activeLocationParcel.section_name }} {{ props.activeLocationParcel.land_no }}</output></label>
        ...
        <p v-else-if="props.activeLocation?.is_benchmark_location" class="empty-copy">請先在左側建立這個比準地點的宗地資料，系統就會自動帶入來源宗地。</p>
      </section>
    </div>
  </section>
</template>
